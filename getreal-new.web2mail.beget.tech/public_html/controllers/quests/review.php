...
if ($user->is_logged() == true) {
	...
	$search_last_request = $pdo->prepare('SELECT date_created, status FROM quest_review WHERE user_id = ? ORDER BY date_created DESC LIMIT 1');
	$search_last_request->execute(array($user->id));

	$last_requets = $search_last_request->fetch();

	if ($last_requets === false || $last_requets['date_created'] < (time() - 432000)) {
		$insert_link = $pdo->prepare('INSERT INTO quest_review (user_id, link, date_created) VALUES (?, ?, ?)');
