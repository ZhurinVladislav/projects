	...
	if (preg_match('/^([0-9]+)\.[0-9]{1,8}$/', $amount_enter) === 1 || preg_match('/^([0-9]+)$/', $amount_enter) === 1) {
		...
		switch ($_POST['items']['payment_system']) {
			...
			case 'bitcoin':
				...
				$amount_get = format_money($amount_enter);
				...
				// $result['result']['address'] = '1GpHUHppAXCeN4HrPDUTbJpxVNCG6Tbhjh';
			...
			case 'doge':
				...
				//	временный костыль, пока биток не работает
				// $result['result']['amount'] = $amount_enter;
				// $result['result']['address'] = 'DFBQbRDyDmjXsF3nXJawwtjY5F2325m172';
			...
			case 'ethereum':
				...
				//	временный костыль, пока биток не работает
				// $result['result']['amount'] = $amount_enter;
				// $result['result']['address'] = '0x865dbd23De1aB189f21c203356468F36537DF8Ba';
