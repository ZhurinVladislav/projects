...
function logs_get_authorizations(int $user_id, $table, int $page = 1, int $limit = 0) {
	global $pdo;
	...
	if ($count != 0) {
		...
		if ($table == 'replenishments' || $table == 'withdrawal') {
			$sort_date = 'date_created';
		} else {
			$sort_date = 'date';
		}
		$items_list = $pdo->prepare('SELECT * FROM '.$table.' WHERE user_id = ? ORDER BY '.$sort_date.' DESC LIMIT ?, ?');
