...
import { TCompany, TCompanyService, TFetchCompanyRating, TFetchCompanyServiceLink, TFetchCompanySocialSchema, TFetchCompanyWorkday } from '@/types';
import { TStoreCategoryServices } from '@/types/CategoryServices/type';
import { TPageSimple } from '@/types/pages/types';
import { TService } from '@/types/Service/type';
import { AnimatePresence, motion } from 'framer-motion';
import { Clock, Link, Plus, Share2, Star, X } from 'lucide-react';
...
const CompanyForm: React.FC<IProp> = ({ dataPages, dataCategories, dataServices, obj }) => {
  ...
  const [ratings, setRatings] = useState<TFetchCompanyRating[]>(obj?.ratings ?? []);
  ...
  // 🔹 ФУНКЦИИ ДЛЯ РЕЙТИНГОВ (С ВЫПАДАЮЩИМ СПИСКОМ ПЛАТФОРМ)
  const addRating = () => {
    setRatings(prev => [
      ...prev,
      {
  ...
  const updateRating = (index: number, field: keyof TFetchCompanyRating, value: string | number) => {
    setRatings(prev => prev.map((rating, i) => (i === index ? { ...rating, [field]: value } : rating)));
    setIsChanged(true);
  };
  ...
  // 🔹 Функция для получения доступных платформ (исключая уже выбранные)
  const getAvailablePlatforms = () => {
    const usedPlatforms = ratings.map(r => r.type).filter(Boolean);
    return RATING_PLATFORMS.filter(platform => !usedPlatforms.includes(platform.value));
  };
  ...
  useEffect(() => {
    ...
    const fieldsChanged =
      ...
      JSON.stringify(ratings) !== JSON.stringify(obj.ratings ?? []);

    setIsChanged(fieldsChanged);
  }, [pageId, title, introText, imgAlt, phone, siteUrl, experience, address, mapLink, promo, categories, services, mainImage, gallery, workdays, socials, servicesLinks, ratings, obj]);
    ...
    try {
      let payload: any = {
        ...
        ratings: ratings.map(rating => ({
          id: rating.id, // для обновления существующих
          type: rating.type,
          link: rating.link,
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4.5">
      ...
      <div className="rounded-lg border border-(--secondary-color) bg-(--bg-op-1-color) p-4">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Star size={18} className="text-(--accent-color)" />
            <h3 className="text-lg font-semibold">Рейтинги на платформах</h3>
          </div>
          <Button type="button" onClick={addRating} className="flex items-center gap-2" disabled={getAvailablePlatforms().length === 0}>
            <Plus size={16} />
            Добавить рейтинг
          </Button>
        </div>

        {getAvailablePlatforms().length === 0 && ratings.length > 0 && (
          <div className="mb-3 rounded bg-blue-50 p-3">
            <p className="text-sm text-blue-700">Все платформы уже добавлены. Удалите одну из платформ, чтобы добавить новую.</p>
          </div>
        )}
        ...
        <div className="space-y-3">
          {ratings.map((rating, index) => (
            <motion.div
              ...
              className="grid grid-cols-1 gap-3 rounded border border-(--secondary-color) bg-(--bg-op-1-color) p-3 md:grid-cols-4"
            >
              {/* Платформа */}
              <div className="md:col-span-1">
                <Dropdown
                  ...
                  items={[
                    // Сначала показываем выбранную платформу (если есть)
                    ...(rating.type ? RATING_PLATFORMS.filter(p => p.value === rating.type) : []),
                    // Затем доступные платформы
                    ...getAvailablePlatforms(),
                  ]}
                  selectedValue={rating.type}
                  onSelect={value => updateRating(index, 'type', value)}
                  className="w-full"
                />
              </div>
              ...
              <div className="md:col-span-1">
                <input
                  ...
                  value={rating.link || ''}
                  onChange={e => updateRating(index, 'link', e.target.value)}
                  className="w-full rounded border border-(--secondary-color) p-2 text-sm"
                />
              </div>
              ...
              <div className="md:col-span-1">
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="Рейтинг"
                    value={rating.rating || ''}
                    onChange={e => updateRating(index, 'rating', parseFloat(e.target.value) || 0)}
                    className="w-full rounded border border-(--secondary-color) p-2 text-sm"
                  />
                  <span className="text-sm text-(--secondary-color)">/5</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 md:col-span-1">
                <input
                  ...
                  value={rating.total_reviews || ''}
                  onChange={e => updateRating(index, 'total_reviews', parseInt(e.target.value) || 0)}
                  className="w-full rounded border border-(--secondary-color) p-2 text-sm"
                />
                <button type="button" className="flex h-8 w-8 items-center justify-center rounded-sm bg-(--error-color) text-white" onClick={() => removeRating(index)}>
                  <X size={14} />
                </button>
              </div>
            </motion.div>
          ))}
          {ratings.length === 0 && <p className="py-4 text-center text-sm text-(--secondary-color)">Рейтинги с платформ не добавлены</p>}
        </div>
      </div>
