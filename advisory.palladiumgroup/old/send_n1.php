        ...
        if(count($conts)>0){//есть такие контакты
            ...
            if(count($activeLeads)>0){ //есть активные сделки
                foreach ($activeLeads as $activeLead) {
                    $response = addLeadTask($activeLead);
                    if($response['responseCode']==200){
                        $return['status']  = 'ok';
                        $return['title']   = check_lang('Спасибо! <br> Ваша заявка отправлена.', 'Thank you! <br> Your request has been sent.');
                        $return['message'] = check_lang('В ближайшее время мы с Вами свяжемся!', 'We will contact you shortly');
                        $return['id'] = $activeLead;
                    }
                }
            }
 ...
 * @return array
 */
function addLeadTask($leadId){
    $task = [[
        "text" => "Внимание! Клиент отправил повторную заявку ".date("Y/m/d H:i").", нужно с ним связаться!",
...
function addLead ($contactId=null){
   ...
   /* if ($lang){
    ...
    $lead['custom_fields_values'][]=array('field_id'=>679024,'values'=>array(array('value'=>$amo_url)));
    /* end custom fields */
