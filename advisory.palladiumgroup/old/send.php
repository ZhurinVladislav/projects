...
function sendFb (){
    ...
    $pixelId = '1941310012898840';
    $token = 'EAAISGheNvJMBAL9Q6PB1zS6Hf0VbbZB6W4EvRhv7jSUPviRSepmJXoEZB9Vos64LfCS9GRQfavQZA21pfCZC22sNOuBqcIJH7hP8UiXAamfK9VBzdi4VXjN8YaG7NCayEa8mngQ9BrxZBBV9ZCckuzKdnzu4ayrpSlr0y3BSH8xZAbXRYU9eyJONZCCCGFB3ZCvkZD';

    if($place == 'advisorybank' or $place == 'advisorymain'){
        $lng = $_POST['lang'] === 'en' ? 'en/' : '';
        if($place == 'advisorymain'){
            $esu = 'https://advisory.palladiumgroup.ae/'.$lng;
        }
        elseif($place == 'advisorybank'){
            $esu = 'https://advisory.palladiumgroup.ae/bank/'.$lng;
        }

        $hashedPhone = hash('sha256', $amo_phone);
...
*/
        /*  $curl=curl_init();
          curl_setopt($curl,CURLOPT_RETURNTRANSFER,true);
         // curl_setopt($curl,CURLOPT_USERAGENT,'amoCRM-API-client/1.0');
          curl_setopt($curl,CURLOPT_URL,'https://graph.facebook.com/v16.0/'.$pixelId.'/events?access_token='.$token);
          curl_setopt($curl,CURLOPT_CUSTOMREQUEST,'POST');
          curl_setopt($curl,CURLOPT_POSTFIELDS,json_encode($data));
          curl_setopt($curl,CURLOPT_HTTPHEADER,array('Content-Type: application/json'));
          curl_setopt($curl,CURLOPT_HEADER,false);
...
*/
        ...
        $data = array( // main object
            ...
            "access_token" => "EAAISGheNvJMBAL9Q6PB1zS6Hf0VbbZB6W4EvRhv7jSUPviRSepmJXoEZB9Vos64LfCS9GRQfavQZA21pfCZC22sNOuBqcIJH7hP8UiXAamfK9VBzdi4VXjN8YaG7NCayEa8mngQ9BrxZBBV9ZCckuzKdnzu4ayrpSlr0y3BSH8xZAbXRYU9eyJONZCCCGFB3ZCvkZD"

        );

        $dataString = json_encode($data);
     //   $ch = curl_init('https://graph.facebook.com/v16.0/'.$pixelId.'/events?access_token='.$token);
        $ch = curl_init('https://graph.facebook.com/v16.0/'.$pixelId.'/events');
        ...
        $responseCode = intval($responseCode);
        $responseData = json_decode($out,true);
        file_put_contents(__DIR__ . "/log/request_log.txt", date("Y/m/d H:i:s")." FB_response: ".$responseCode." ".json_encode($responseData) . PHP_EOL, FILE_APPEND);
