...
if ($_POST['action'] == 'get_page_num') {
	$suggestions = suggestions_get_list($_POST['data']['page'], 10);
	if ($suggestions != 'empty') {
?>
		<div class="table-default" data-placeholder="result_suggestions">
			<div class="table-row table-header">
				<div class="table-column" style="width: 20%">Логин</div>
				<div class="table-column" style="width: 60%">Предложение</div>
				<div class="table-column" style="width: 10%">Дата</div>
				<div class="table-column" style="width: 10%">Удалить</div>
			</div>
			<?php
			foreach ($suggestions['items'] as $key => $value) {
			...
			?>
			<div class="pagination" data-result="result_suggestions">
			<?php
...
if ($_POST['action'] == 'add_answer') {
	$suggestion_id = $_POST['data']['id'];
	$text = clean_string($_POST['data']['text']);
	$write_answer = $pdo->prepare('UPDATE suggestions SET answer = ? WHERE id = ?');
	if ($write_answer->execute(array($text, $suggestion_id))) {
		echo 1;
	} else {
		echo 0;
	}
...
if ($_POST['action'] == 'delete') {
	$delete = suggestions_delete($_POST['id']);
	if ($delete === true) {
		echo 1;
	} else {
		echo 0;
...
?>
		<div data-placeholder="result_suggestions">
<?php
	}
	...
	$suggestions = suggestions_get_list(1, 10);
	if ($suggestions != 'empty') {
?>
...
?>
		<p class="description">Опубликованных предложений нет.</p>
<?php
	}
	if (!isset($_POST['data']['page'])) {
?>
