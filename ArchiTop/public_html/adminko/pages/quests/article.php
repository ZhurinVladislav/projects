...
if ($_POST['action'] == 'accept') {
	...
	if ($reward >= 0.00002000 && $reward <= 0.00300000) {

		if ($accept_quest = $pdo->query('UPDATE quest_article SET status = 1, date_confirm = '.time().' WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_article WHERE id = '.$item_id.' LIMIT 1');
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_article SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
?>
		Выдаем награды в размере 0.00002000 - 0.00300000 <?= $coin; ?>
		<br><br>
...
<?php
	...
	$quests = quests_get_list('article', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
				...
				<div class="table-row">
					...
					?>
							<form data-accept-quest="<?= $value['id']; ?>">
								<input class="form-item__input" type="text" name="reward" style="width: 110px; height: 28px" value="0.00002000">
