...
if ($_POST['action'] == 'accept') {
	...
	if ($reward == 0.00000100) {
		...
		if ($accept_quest = $pdo->query('UPDATE quest_tg_channel SET status = 1, date_confirm = '.time().' WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_tg_channel WHERE id = '.$item_id.' LIMIT 1');
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_tg_channel SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
<?php
	...
	$quests = quests_get_list('tg_channel', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
		<div class="table-default">
			<div class="table-row table-header">
				<div class="table-column" style="width: 10%">Логин</div>
				<div class="table-column" style="width: 60%">Ник</div>
