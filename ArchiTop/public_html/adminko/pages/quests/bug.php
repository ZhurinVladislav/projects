...
if ($_POST['action'] == 'accept') {
	...
	if ($reward >= 0.00000015 && $reward <= 0.00100000) {

		if ($accept_quest = $pdo->query('UPDATE quest_bug SET status = 1, date_confirm = '.time().' WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_bug WHERE id = '.$item_id.' LIMIT 1');
			$get_user_id = $get_user_id->fetch();

			update_balance($get_user_id['user_id'], 'buy', '+', $reward);
			write_log($get_user_id['user_id'], '04011', $reward);
			echo 1;
		} else {
			echo 0;
		}
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_bug SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
if ($_POST['action'] == 'wait') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_bug SET status = 3 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
?>
		Выдаем награды в размере 0.00000015 - 0.00100000 <?= $coin; ?>
		<br><br>
...
<?php
	...
	$quests = quests_get_list('bug', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
		<div class="table-default">
			<div class="table-row table-header">
				<div class="table-column" style="width: 10%">Логин</div>
				<div class="table-column" style="width: 55%">Сообщение</div>
			...
			?>
				<div class="table-row <?= $wait; ?>">
					...
					<div class="table-column" style="width: 55%"><?= stripslashes(htmlspecialchars_decode($value['message'])); ?></div>
					<div class="table-column center" style="width: 25%">
					<?php
						if ($quest_status == 0) {
					?>
							<form data-accept-quest="<?= $value['id']; ?>">
								<input class="form-item__input" type="text" name="reward" style="width: 110px; height: 28px" value="0.00000015">
