...
const CompanyDeleteButton: React.FC<IProps> = ({ id, title = 'Удалить элемент', description = 'Вы уверены, что хотите удалить?', triggerLabel = 'Удалить' }) => {
  const router = useRouter();

  const handleDelete = async () => {
    await Api.fetchDeleteCompany(id);
