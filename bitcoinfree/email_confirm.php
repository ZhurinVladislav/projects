...
if (!is_bool($finded = $find_email->fetch())) {
	...
	$true_hash = md5($given_email.'emailconfirm'.$email_salt);

	if ($given_hash === $true_hash) {
		$pdo->query('UPDATE users SET email_confirmed = 1 WHERE id = '.$finded['id'].' LIMIT 1');
		header('Location: '.$root_url.'tutorial');
	}
