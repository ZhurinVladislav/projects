...
if ($user->is_logged() == true) {
	...
	if ($nickname != '') {
		...
		$search_last_request = $pdo->query('SELECT * FROM quest_telegram_chat WHERE user_id = '.$user->id.' LIMIT 1');
		if (!is_bool($row = $search_last_request->fetch())) {
			$can_send_report = false;
		}
		if ($can_send_report == true) {

			$insert_link = $pdo->prepare('INSERT INTO quest_telegram_chat (user_id, nickname, date_created) VALUES (?, ?, ?)');
