...
if ($_POST['action'] == 'accept') {
	...
	if ($reward >= 0.00000015 && $reward <= 0.00100000) {
		...
		if ($accept_quest = $pdo->query('UPDATE quest_youtube SET status = 1 WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_youtube WHERE id = '.$item_id.' LIMIT 1');
			$get_user_id = $get_user_id->fetch();

			update_balance($get_user_id['user_id'], 'buy', '+', $reward);
			write_log($get_user_id['user_id'], '04021', $reward);
			echo 1;
		} else {
			echo 0;
		}
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_youtube SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
<?php
	...
	$quests = quests_get_list('youtube', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
		<div class="table-default">
			...
			?>
				<div class="table-row">
					...
					<div class="table-column" style="width: 60%"><a href="<?= $value['link']; ?>" target="_blank"><?= $value['link']; ?></a></div>
