...
if ($_POST['action'] == 'accept') {
	...
	if ($reward == 0.00002000) {

		if ($accept_quest = $pdo->query('UPDATE quest_vk_repost SET status = 1 WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_vk_repost WHERE id = '.$item_id.' LIMIT 1');
			$get_user_id = $get_user_id->fetch();

			update_balance($get_user_id['user_id'], 'buy', '+', $reward);
			write_log($get_user_id['user_id'], '04041', $reward);
			echo 1;
		} else {
			echo 0;
		}
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_vk_repost SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
?>
		Выдаем награды в размере 0.00002000 <?= $coin; ?>
		<br><br>
...
<?php
	...
	$quests = quests_get_list('vk_repost', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
			...
			?>
				<div class="table-row">
					...
					<div class="table-column" style="width: 60%">
						<a href="<?= $value['link_sended']; ?>" target="_blank"><?= $value['link_sended']; ?></a>
					</div>
					...
					?>
							<form data-accept-quest="<?= $value['id']; ?>">
								<input class="form-item__input" type="text" name="reward" style="width: 110px; height: 28px" value=" 0.00002000" disabled>
