...
if ($_POST['action'] == 'accept') {
	...
	if ($reward >= 0.00000030 && $reward <= 0.00100000) {

		if ($accept_quest = $pdo->query('UPDATE quest_telegram_channel SET status = 1, date_confirm = '.time().' WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_telegram_channel WHERE id = '.$item_id.' LIMIT 1');
			$get_user_id = $get_user_id->fetch();

			update_balance($get_user_id['user_id'], 'buy', '+', $reward);
			write_log($get_user_id['user_id'], '04061', $reward, '', false);
			echo 1;
		} else {
			echo 0;
		}
...
if ($_POST['action'] == 'accept-all') {

	if ($_POST['data']['accept'] == 'all') {

		$reward = format_money(0.00000030);

		$get_all_quests = $pdo->query('SELECT * FROM quest_telegram_channel WHERE status = 0');
		$all_quests = $get_all_quests->fetchAll();

		foreach ($all_quests as $key => $request) {
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_telegram_channel SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
if ($_POST['action'] == 'wait') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_telegram_channel SET status = 3 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
?>
		Выдаем награды в размере 0.00000030 <?= $coin; ?>
		<br><br>
...
<?php
	...
	$quests = quests_get_list('telegram_channel', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
		<form data-accept-all-quest>
			<button class="button-d">Подтвердить все заявки</button>
		</form>
		<br>
		<div class="table-default">
			<div class="table-row table-header">
				<div class="table-column" style="width: 10%">Логин</div>
				<div class="table-column" style="width: 55%">Ник в Telegram</div>
				<div class="table-column" style="width: 25%">Кнопки</div>
			</div>
			<?php
			...
			?>
				<div class="table-row <?= $wait; ?>">
					<div class="table-column" style="width: 10%">
						<span data-edit="users/edit_user" data-id="<?= $value['user_id']; ?>">
							<?= $login; ?>
						</span>
					</div>
					<div class="table-column" style="width: 55%"><?= stripslashes(htmlspecialchars_decode($value['nickname'])); ?></div>
					<div class="table-column center" style="width: 25%">
					<?php
						if ($quest_status == 0) {
					?>
							<form data-accept-quest="<?= $value['id']; ?>">
								<input class="form-item__input" type="text" name="reward" style="width: 110px; height: 28px" value="0.00000030">
								<button class="button-d square">&#10003;</button>
							</form>
							<button class="button-d square" data-wait-quest="<?= $value['id']; ?>">&#8545;</button>
