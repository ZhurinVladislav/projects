...
if ($_POST['action'] == 'accept') {
	...
	if ($reward >= 0.00000030 && $reward <= 0.00100000) {
		...
		if ($accept_quest = $pdo->query('UPDATE quest_telegram_chat SET status = 1, date_confirm = '.time().' WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_telegram_chat WHERE id = '.$item_id.' LIMIT 1');
			$get_user_id = $get_user_id->fetch();

			update_balance($get_user_id['user_id'], 'buy', '+', $reward);
			write_log($get_user_id['user_id'], '04051', $reward, '', false);
			echo 1;
		} else {
			echo 0;
		}
...
if ($_POST['action'] == 'accept-all') {
	...
	if ($_POST['data']['accept'] == 'all') {
		...
		$get_all_quests = $pdo->query('SELECT * FROM quest_telegram_chat WHERE status = 0');
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_telegram_chat SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
if ($_POST['action'] == 'wait') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_telegram_chat SET status = 3 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
<?php
	...
	$quests = quests_get_list('telegram_chat', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
