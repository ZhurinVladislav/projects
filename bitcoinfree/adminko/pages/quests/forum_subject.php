...
if ($_POST['action'] == 'accept') {
	...
	if ($reward >= 0.00010000 && $reward <= 0.00100000) {

		if ($accept_quest = $pdo->query('UPDATE quest_forum_subject SET status = 1, date_confirm = '.time().' WHERE id = '.$item_id.' LIMIT 1') !== false) {
			$get_user_id = $pdo->query('SELECT user_id FROM quest_forum_subject WHERE id = '.$item_id.' LIMIT 1');
...
if ($_POST['action'] == 'reject') {
	...
	if ($accept_quest = $pdo->query('UPDATE quest_forum_subject SET status = 2 WHERE id = '.$item_id.' LIMIT 1') !== false) {
		echo 1;
	} else {
		echo 0;
	}
...
?>
		Выдаем награды в размере 0.00010000 - 0.00100000 <?= $coin; ?>
		<br><br>
...
<?php
	...
	$quests = quests_get_list('forum_subject', $quest_status, $page, 30);
	if ($quests != 'empty') {
?>
		<div class="table-default">
			<div class="table-row table-header">
				<div class="table-column" style="width: 10%">Логин</div>
				<div class="table-column" style="width: 60%">Ссылка</div>
			...
			?>
				<div class="table-row">
					...
					<div class="table-column" style="width: 60%">
						<a href="<?= stripslashes(htmlspecialchars_decode($value['link'])); ?>" target="_blank"><?= stripslashes(htmlspecialchars_decode($value['link'])); ?></a>
					</div>
					...
					?>
							<form data-accept-quest="<?= $value['id']; ?>">
								<input class="form-item__input" type="text" name="reward" style="width: 110px; height: 28px" value="0.00010000">
