...
if ($user->is_logged() == false) {
	// $get_last_user_id = $pdo->query('SELECT id FROM users ORDER BY id DESC LIMIT 1');
	// $last_user_id = $get_last_user_id->fetch();
	// $last_user_id = $last_user_id['id'];
	// $_POST['login'] = 'user' . ($last_user_id + 1);
