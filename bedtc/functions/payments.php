...
function payments_mark_payed(int $id) {
	...
	if ($payment['action_percent'] > 0) {
		...
		$payment_amount_get = format_money(round_money($payment['amount_get'] + ($payment['amount_get'] * $payment['action_percent'] / 100)));
	}
	...
	$charge_payment->execute(array($payment_amount_get, $payment_amount_get, $payment['user_id']));
